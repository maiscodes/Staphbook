...
app.use(
  "/filesaver_scripts",
  express.static(__dirname + "/node_modules/filesaver/")
);
...
app.get("/", function (req, res) {
  ...
  if (req.session.userStatus == "loggedIn") {
    ...
    client.query(
      "SELECT * FROM user_favorites WHERE email='" + email + "' LIMIT 4",
      (err, fav_results) => {
        ...
        if (fav_results.rows.length > 0) {
          ...
          if (fav_results.rows.length > 1) {
            selectSQL +=
              " OR  sample_id = " + fav_results.rows[1].sample_id + "";
          }
          if (fav_results.rows.length > 2) {
            selectSQL +=
              " OR  sample_id = " + fav_results.rows[2].sample_id + "";
          }
          if (fav_results.rows.length > 3) {
            selectSQL +=
              " OR  sample_id = " + fav_results.rows[3].sample_id + "";
          }
          ...
          client.query(selectSQL + ");", (err, favorites) => {
            ...
            client.query(
              "SELECT name FROM sample_sample WHERE id=" +
                fav_results.rows[fav_results.rows.length - 1].sample_id +
                "",
              (err, result_sample_sample) => {
                  ...
                  (err, result_weighted_distances) => {
                    ...
                    client.query(
                      "SELECT mlst_mlst.st, sample_metadata.sample_id, metadata->>'country' AS country, " +
                        ...
                        result_weighted_distances.rows[2].comparison_sample +
                        "' " +
                        "OR sample_sample.name='" +
                        result_weighted_distances.rows[3].comparison_sample +
                        "' " +
                        "OR sample_sample.name='" +
                        result_weighted_distances.rows[4].comparison_sample +
                        "'",
                      (err, suggested) => {
...
app.get("/result", function (req, res) {
  ...
  if (userLoggedIn) {
    ...
    let email = decodeURIComponent(value);
    client.query(
      "SELECT * FROM user_favorites WHERE email='" +
        email +
        "' AND sample_id=" +
        sampleID +
        "",
  ...
  // Tags
  client.query(
    "SELECT tag_id FROM tag_tosample WHERE sample_id=" + sampleID + "",
    (err, result_tag_tosample) => {
      //console.log(err, result_tag_tosample);

      try {
        let tagID = result_tag_tosample.rows[0].tag_id;
        client.query(
          "SELECT * FROM tag_tag WHERE id=" + tagID + "",
          (err, result_tag_tag) => {
            // Sample Name
            client.query(
              "SELECT name FROM sample_sample WHERE id=" + sampleID + "",
              (err, result_sample_sample) => {
                console.log(err, result_tag_tosample);
                let sampleName = result_sample_sample.rows[0].name;
                ...
                client.query(
                  ...
                  (err, result_weighted_distances) => {
                    ...
                    // Metadata
                    client.query(
                      "SELECT * FROM sample_metadata WHERE sample_id=" +
                        sampleID +
                        "",
                      (err, result_sample_metadata) => {
                        // BLASTquery
                        client.query(
                          "SELECT * FROM staphopia_blastquery",
                          (err, result_blastquery) => {
                            // Sequencing Metrics
                            client.query(
                              "SELECT * FROM sequence_summary WHERE sample_id=" +
                                sampleID +
                                "",
                              (err, result_sequence_summary) => {
                                //console.log(err, result_sequence_summary);

                                // Assembly Metrics
                                client.query(
                                  "SELECT * FROM assembly_summary WHERE sample_id=" +
                                    sampleID +
                                    "",
                                  (err, result_assembly_summary) => {
                                    //console.log(err, result_assembly_summary);

                                    // MLST
                                    client.query(
                                      "SELECT * FROM mlst_mlst WHERE sample_id=" +
                                        sampleID +
                                        "",
                                      (err, result_mlst_mlst) => {
                                        // SCCmec Primer Hits
                                        client.query(
                                          "SELECT * FROM sccmec_primers WHERE sample_id=" +
                                            sampleID +
                                            "",
                                          (err, result_sccmec_primers) => {
                                            // SCCmec Subtype Hits
                                            client.query(
                                              "SELECT * FROM sccmec_subtypes WHERE sample_id=" +
                                                sampleID +
                                                "",
                                              (err, result_sccmec_subtypes) => {
                                                // SCCmec Protien Hits
                                                client.query(
                                                  "SELECT * FROM sccmec_proteins WHERE sample_id=" +
                                                    sampleID +
                                                    "",
                                                  (
                                                    err,
                                                    result_sccmec_proteins
                                                  ) => {
                                                    // Samples with the same Sequence Type
                                                    let st =
                                                      result_mlst_mlst.rows[0]
                                                        .st;
                                                    client.query(
                                                      "SELECT mlst_mlst.st, sample_metadata.sample_id, metadata->>'country' AS country, " +
                                                        "sample_metadata.metadata->>'strain' AS strain, sample_metadata.metadata->>'host' AS host, " +
                                                              ...
                                                              (
                                                                ...
                                                                client.query(
                                                                  "SELECT mlst_mlst.st, sample_metadata.sample_id, metadata->>'country' AS country, " +
                                                                    ...
                                                                    "INNER JOIN mlst_mlst ON sample_metadata.sample_id=mlst_mlst.sample_id " +
                                                                    "WHERE sample_metadata.sample_id IN (SELECT sample_id FROM sample_metadata " +
                                                                    "WHERE metadata->>'isolation_source' = '" +
                                                                    iso +
                                                                    "')",
                                                                  (
                                                                    err,
                                                                      ...
                                                                      .then(
                                                                        (
                                                                          ...
                                                                          knex
                                                                            ...
                                                                            .innerJoin(
                                                                              "sample_sample",
                                                                              "weighted_distance.comparison_sample",
                                                                              "sample_sample.name"
                                                                            )
                                                                            .innerJoin(
                                                                              "sample_metadata",
                                                                            ...
                                                                            .where(
                                                                              "weighted_distance.selected_sample",
                                                                              "=",
                                                                              sampleName[0]
                                                                                .name
                                                                            )
                                                                              ...
                                                                              (
                                                                                ...
                                                                                rows.forEach(
                                                                                  function (
                                                                                    ...
                                                                                    detailedRow.host =
                                                                                      row.metadata.host;
                                                                                    detailedRow.isolation_source =
                                                                                      row.metadata.isolation_source;
                                                                                    mainRelatedSampleDetails.push(
                                                                                      detailedRow
                                                                                    );
                                                                                  }
                                                                                );
                                                                                ...
                                                                                res.render(
                                                                                  "pages/result",
                                                                                  {
                                                                                    ...
                                                                                    sample_metadata:
                                                                                      result_sample_metadata.rows,
                                                                                    mlst_mlst:
                                                                                      result_mlst_mlst.rows,
                                                                                    userLoggedIn: userLoggedIn,
                                                                                    same_hosts:
                                                                                      same_host.rows,
                                                                                    same_locations:
                                                                                    ...
                                                                                    sequence_summary:
                                                                                      result_sequence_summary.rows,
                                                                                    same_isolations:
                                                                                      same_isolation.rows,
                                                                                    sccmec_primers:
                                                                                      result_sccmec_primers.rows,
                                                                                    assembly_summary:
                                                                                      result_assembly_summary.rows,
                                                                                    sccmec_subtypes:
                                                                                      result_sccmec_subtypes.rows,
                                                                                    sccmec_proteins:
                                                                                      result_sccmec_proteins.rows,
                                                                                    weighted_distance:
                                                                                      result_weighted_distances.rows,
    ...
    if (option == "Sequence") {
      ...
      client.query(
        "SELECT mlst_mlst.st, sample_metadata.sample_id, metadata->>'country' AS country, " +
          ...
          "FROM sample_metadata INNER JOIN mlst_mlst ON sample_metadata.sample_id=mlst_mlst.sample_id " +
          "WHERE sample_metadata.sample_id IN (SELECT sample_id FROM sample_metadata " +
          "WHERE metadata->>'isolation_source' ILIKE '%" +
          input +
          "%')",
        (err, result_samples) => {
          console.log(err, result_samples);
...
app.get("/advSearchResults", function (req, res) {
  ...
  if (
    ...
    if (startDate != "" || endDate != "") {
      if (endDate == "") {
        selectSQL +=
          " CAST(metadata->>'collection_date' AS DATE) >= '" + startDate + "'";
      } else if (startDate == "") {
        selectSQL +=
          " CAST(metadata->>'collection_date' AS DATE) <= '" + endDate + "'";
      } else {
        selectSQL +=
          " CAST(metadata->>'collection_date' AS DATE) >= '" +
          startDate +
          "' AND  CAST(metadata->>'collection_date' AS DATE) <= '" +
          endDate +
          "'";
      }
    ...
    if (sourceInput != "") {
      selectSQL +=
        " metadata->>'isolation_source' ILIKE '%" + sourceInput + "%' ";
    }
    console.log(selectSQL);
    ...
    client.query(selectSQL + ";", (err, result_samples) => {
      console.log(err, result_samples);
      number = result_samples.rows.length;
      res.render("pages/advSearchResults", {
        samples: result_samples.rows,
        number: number,
        userLoggedIn: userLoggedIn,
      });
...
app.get("/predictSeq", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT st FROM mlst_mlst WHERE CAST(st AS CHAR(10)) LIKE '" +
      sequence +
      "%' LIMIT 10",
...
app.get("/predictSam", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT sample_id FROM mlst_mlst WHERE CAST(sample_id AS CHAR(10)) LIKE '" +
      sample +
      "%' LIMIT 10",
    function (err, result, fields) {
      if (err) throw err;
...
app.get("/predictLoc", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT metadata->>'country' AS country FROM sample_metadata WHERE metadata->>'country' ILIKE '%" +
      location +
      "%' LIMIT 10",
    function (err, result, fields) {
      if (err) throw err;
...
app.get("/predictStr", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT metadata->>'strain' AS strain FROM sample_metadata WHERE metadata->>'strain' ILIKE '%" +
      strain +
      "%' LIMIT 10",
    function (err, result, fields) {
      if (err) throw err;
...
app.get("/predictHost", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT metadata->>'host' AS host FROM sample_metadata WHERE metadata->>'host' ILIKE '%" +
      host +
      "%' LIMIT 10",
    function (err, result, fields) {
      if (err) throw err;
...
app.get("/predictIso", function (req, res) {
  ...
  client.query(
    "SELECT DISTINCT metadata->>'isolation_source' AS isolation_source FROM sample_metadata WHERE metadata->>'isolation_source' ILIKE '%" +
      source +
      "%' LIMIT 10",
    function (err, result, fields) {
      if (err) throw err;
...
app.get("/favourites", function (req, res) {
  ...
  if (req.session.userStatus == "loggedIn") {
    ...
    let email = decodeURIComponent(value);
    client.query(
      "SELECT * FROM user_favorites WHERE email='" + email + "'",
      (err, fav_results) => {
        console.log(err, fav_results);
        if (fav_results.rows.length > 0) {
          haveFavs = true;
          ...
          for (let i = 0; i < fav_results.rows.length; i++) {
            ...
            selectSQL +=
              " OR sample_id = " + fav_results.rows[i].sample_id + "";
          }
...
app.get("/group", function (req, res) {
  ...
  if (req.session.userStatus == "loggedIn") {
    ...
    let email = decodeURIComponent(value);
    client.query(
      "SELECT * FROM group_samples WHERE email='" + email + "'",
      (err, groupSamp_results) => {
        ...
        if (groupSamp_results.rows.length > 0) {
          ...
          for (let i = 0; i < groupSamp_results.rows.length; i++) {
            if (i == 0) {
              selectSQL +=
                " sample_id = " + groupSamp_results.rows[i].sample_id + "";
            }
            selectSQL +=
              " OR sample_id = " + groupSamp_results.rows[i].sample_id + "";
          }
          ...
          client.query(selectSQL + ");", (err, group_samples) => {
            ...
            res.render("pages/groups", {
              userLoggedIn: userLoggedIn,
              group_samples: group_samples.rows,
              haveGroupSamps: haveGroupSamps,
            });
          });
        } else {
...
app.get("/createGroup", function (req, res, next) {
  res.render("createNewGroup");
});

app.post("/createGroup", function (req, res, next) {
  var sql = `INSERT INTO groups (group_id, owner, name, description )
...
app.post("/createAccount", function (req, res) {
  ...
  if (email != "") {
    bcrypt.hash(req.body.password, 10, function (err, hashedPassword) {
      client.query(
        "SELECT * FROM registered_users WHERE email='" + req.body.email + "'",
        (err, result_registered_users) => {
          console.log(err, result_registered_users);
          ...
          if (result_registered_users.rows.length != 0) {
            ...
            client.query(
              "INSERT INTO registered_users (email, password, organisation, occupation) VALUES ('" +
                email +
                "','" +
                hashedPassword +
                "','" +
...
app.post("/result", function (req, res) {
  ...
  if (isFavourited == 0) {
    if (userLoggedIn) {
      ...
      client.query(
        ...
        (err, fav_results) => {
          ...
          if (fav_results.rows.length < 1) {
            client.query(
              "INSERT INTO user_favorites (email, sample_id) VALUES ('" +
                email +
                "','" +
                sampleID +
                "')",
    ...
    isFavourited = 1;
  } else if (isFavourited == 1) {
    // isFavourited is set to 'true', so set it to 'false'
    if (userLoggedIn) {
      let value = req.session.userEmail;
      let email = decodeURIComponent(value);
      console.log(
      ...
      client.query(
        ...
        (err, fav_results) => {
          ...
          if (fav_results.rows.length > 0) {
            client.query(
              "DELETE FROM user_favorites WHERE email='" +
                email +
                "' AND sample_id=" +
                sampleID +
                "",
            ...
            (err, result_sample_sample) => {
              ...
              client.query(
                ...
                (err, result_weighted_distances) => {
                  ...
                  console.log(
                    err,
                    JSON.stringify(result_weighted_distances.rows)
                  );
              ...
              (err, fav_results) => {
                ...
                if (fav_results.rows.length > 0) {
                  ...
                  for (let i = 0; i < fav_results.rows.length; i++) {
                    if (i == 0) {
                      selectSQL +=
                        " sample_id = " + fav_results.rows[i].sample_id + "";
                    } else {
                      selectSQL +=
                        " OR sample_id = " + fav_results.rows[i].sample_id + "";
                    }
